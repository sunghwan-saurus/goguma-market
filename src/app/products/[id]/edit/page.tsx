...
export default function EditProductPage() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      setForm({
        title: data.title ?? '',
        price: data.price ? data.price.toLocaleString('ko-KR') : '',
  ...
  if (fetching) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#FFF6E8' }}>
        <p style={{ color: '#A0704A' }}>불러오는 중...</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen" style={{ background: '#FFF6E8' }}>
      ...
      <header className="sticky top-0 z-10" style={{ background: '#5C2D0E', borderBottom: '1px solid #3D1A00' }}>
        <div className="max-w-screen-sm mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-base font-bold" style={{ color: '#FFD07B' }}>상품 수정</h1>
        </div>
      </header>
          ...
          <div className="flex gap-2">
            {(['판매중', '예약중', '판매완료'] as const).map(s => (
              <button
                ...
                style={{
                  background: form.status === s ? '#C94E00' : '#fff',
                  color: form.status === s ? '#fff' : '#A0704A',
                  border: form.status === s ? '1.5px solid #C94E00' : '1.5px solid #FFE4BA',
                }}
              >
                {s}
              </button>
